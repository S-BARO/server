...
interface UserSwagger {
    ...
    @Operation(
        summary = "전화번호 수정",
        description = "현재 로그인한 회원의 전화번호를 수정합니다.",
        requestBody = io.swagger.v3.oas.annotations.parameters.RequestBody(
            required = true,
            content = [
                Content(
                    mediaType = APPLICATION_JSON_VALUE,
                    schema = Schema(implementation = PhoneNumberUpdateRequest::class),
                    examples = [
                        ExampleObject(
    ...
    fun updatePhoneNumber(
        @Parameter(hidden = true) userId: Long,
        @RequestBody request: PhoneNumberUpdateRequest,
    ): UserProfileResponse

    /* ──────────────────────────────── 주소 수정 ──────────────────────────────── */
    @Operation(
        summary = "주소 수정",
        description = "현재 로그인한 회원의 주소를 수정합니다.",
        requestBody = io.swagger.v3.oas.annotations.parameters.RequestBody(
            required = true,
            content = [
                Content(
                    ...
                    Content(
                        ...
                        examples = [
                            ExampleObject(
                                ...
                                value = """
                                {
                                  "message": "주소는 최대 500자까지 입력 가능합니다."
                                }
                                """
                            )
                        ]
